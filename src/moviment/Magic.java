...
public class Magic{
	public static void rotate(Cube cube, Camera camera, boolean counterClockWise){
		final Flat faces = camera.getPerspectiveFaces();
		...
		if(cube.hasOnlyPiece()){
			rotateFaces(faces.getFace(0, 0).getPiece(), camera.getPlaneRotation(), counterClockWise);
			return;
		}
		...
		for(int j=0; j < middle; j++)
			for(int i=j; i < max - j; i++){
				final Vector3 begin = faces.getFace(i, j).getPiece().getPosition();
